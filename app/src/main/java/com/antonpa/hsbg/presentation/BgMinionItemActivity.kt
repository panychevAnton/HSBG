...
//    }
    ...
    private fun runCorrectMode() {
        val fragment = when (extraMode) {
            MODE_SHOW_INTENT -> BgMinionItemFragment.createNewInstanceShowItem(extraBgMinionId)
            MODE_ADD_INTENT -> BgMinionItemFragment.createNewInstanceAddItem()
            else -> throw RuntimeException("Unknown extra mode $extraMode")
        }
        supportFragmentManager.beginTransaction()
            .add(R.id.bg_minion_container, fragment)
            .commit()
    }
